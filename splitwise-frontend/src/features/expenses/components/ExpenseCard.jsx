    ...
    if (Array.isArray(expense.splitDetails) && expense.splitDetails.length > 0) {
      return new Set(
        expense.splitDetails
          .map((item) => item?.userId?._id || item?.userId || item?.user?._id)
          .filter(Boolean)
          .map((id) => String(id))
      ).size;
    }
    if (Array.isArray(expense.participants)) return expense.participants.length;
    return 0;
  })();
      ...
      <>
        <Card className="transition-colors hover:bg-muted/30">
          <CardContent className="p-4">
            <div className="flex flex-col  md:gap-4 sm:flex-row sm:items-start sm:justify-between">
              <button
                type="button"
                className="flex min-w-0 flex-1 items-start gap-x-3 text-left"
              ...
              >
                <div className=" flex-1">
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    Your share:{" "}
                    {formatCurrency(yourShare, expense.currency || "INR")}
                  </p>
                </div>
              </button>
